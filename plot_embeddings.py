...
def plot_embeddings_with_hover(df_plot, data_column, color_column, options, column_name, title=None):
    ...
    html_file = os.path.join(options.save_dir, column_name, options.languages[0], f'{options.save_prefix}_wrt_{fig_label.get(color_column, color_column)}_{data_column}.html')
    subpath = os.path.join(options.save_dir, column_name, options.languages[0])
